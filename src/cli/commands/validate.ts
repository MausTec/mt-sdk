...
async function doValidate(paths: string[], opts: { api?: string; strict?: boolean; json?: boolean }): Promise<void> {
  // --- Resolve validation targets ---
  let targets: Target[];
  let strictMode = opts.strict ?? false;

  if (paths.length > 0) {
    targets = paths.flatMap(resolvePositional);
  } else {
    // Auto-discover: look for mt-sdk.json workspace config
    const workspace = discoverWorkspace(process.cwd());

    if (workspace) {
      if (!opts.strict && workspace.config.strict) {
        strictMode = true;
      }
  ...
  if (opts.api) {
    try {
      cliManifest = JSON.parse(readFileSync(opts.api, "utf-8")) as ApiDescriptor;
    } catch (e) {
  ...
  for (const target of targets) {
    ...
    if (target.kind === "mtp") {
      ...
      if (!opts.json && transpileErrors.length > 0) {
        if (!multiFile) {
...
export const validateCommand = new Command("validate")
  .description("Validate plugin JSON or .mtp source files")
  .argument("[paths...]", "files or directories to validate (default: auto-discover)")
  .option("--api <file>", "path to a custom API manifest JSON")
